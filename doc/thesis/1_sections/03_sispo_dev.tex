...
The original simulation used the \gls{skimage} and OpenCV library. In order to reduce the number of dependencies, it was decided to benchmark relevant functions of the two libraries. For this comparison, OpenCV functions were used to create the same behaviour as the respective \gls{skimage} function. The benchmark compares the performance of the Gaussian filter and a special case of image resizing using local means. A set of 5 images was selected, these are shown in \ref{sec:app_cvskimage}. Two star images were selected due to the large variance of number of stars. The two selected represent the extreme cases 1804 and 51338 stars.

Two machines were used for the benchmark to eliminate platform dependency.
The laptop has \SI{8}{\giga\byte} \gls{ram}, an Intel i7-6700HQ with 4 cores at \SI{2.6}{\giga\hertz} and is running Windows 10. The workstation computer has \SI{16}{\giga\byte} of \gls{ram}, an Intel i7-8700 with 6 cores at \SI{3.2}{\giga\hertz} and is running a Linux distribution.
...
\begin{figure}[htb]
    ...
    \end{subfigure}
    \caption{Comparison of execution time ratios of Gaussian filtering and resizing 5 different images using OpenCV and \gls{skimage} on 2 different computers. Values larger than one correspond to OpenCV being faster. The mean value of each computer is given in the legend.}
