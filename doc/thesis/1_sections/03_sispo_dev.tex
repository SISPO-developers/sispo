...
The star background is then generated by generating an image with twice the size of final image. The coordinates are converted from the right ascension and declination of each star into pixel coordinates. Afterwards setting the pixel value of each colour channel to the flux calculated from the magnitude. The resulting image is Gaussian filtered and downscaled to the final image size using local means. The alpha values of the entire image are set to one. Star background images are stored in the OpenEXR format.
...
\begin{align}
    \sigma = 0.45 \times \lambda \times \frac{f}{D} \times d \times \frac{1}{pix_l}, \label{eq:comp_sigma}
\end{align}
where $\sigma$ is the standard deviation, $d$ is the XXXXXXX, $\lambda$ is the instrument wavelength, $f$ is the focal length, $D$ is the aperture diameter and $pix_l$ is the length of one side of a pixel. The noisy image is scaled to an interval $[0,1]$ by dividing through the maximum value. In case a point source \gls{sssb} is used, the maximum value is clipped to five times the maximum of the the reference if the maximum value of the merged image is above this threshold. The composed image using the data set shown in Fig. \ref{fig:comp_imageset} is presented Fig. \ref{fig:comp_composed}.
