...
\begin{table}[htb]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{p{0.2\textwidth}|p{0.2\textwidth}|p{0.6\textwidth}}
            ...
            wavelength [\SI{}{\nano\meter}] & float & Center wavelength of the instrument. \\
...
\begin{figure}[htb]
    ...
    \end{subfigure}
    \caption{(a) Raw, smooth \gls{3d} model before applying texturing and displacement shaders. (b) Shader node network overview depicting the shaders used for creating the final \gls{sssb} surface and terrain. Two main trees are visible in the network that are combined in the output, one for shading and the other for displacement. The two main trees are interlinked, i.e. displacement and shading are partial influenced by the same input.}
\end{figure}
...
\begin{figure}[htb]
    ...
    \end{subfigure}
    \caption{Images of intermediate results of the \gls{sispo} reconstruction pipeline for a \SI{100}{\kilo\meter} fly-by scenario of a \SI{1}{\kilo\meter} \gls{sssb}. Starting from the sparse point cloud and ending in the textured \gls{3d} model outlines the step-wise creation of the final result. (a) Sparse point cloud create by a \gls{omvs} \gls{sfm} algorithm. (b) Point cloud densified with \gls{omvs}. (c) Mesh created from the dense point cloud shown in (b) using \gls{omvs}. (d) Refined mesh based on the mesh created in (c) using \gls{mvs}. (e) Mesh textured using \gls{omvs}. (e) Rreference image for comparison with the textured mesh.}
...
\begin{table}[htb]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{l|l|l|l}
            ...
            -i & i & data/input/definition.json & Path to a definition file that defines the settings \\
            ...
            --sim-only & sim\_only & False & Flag to only do simulation step \\
            --sim-render-only & sim\_render\_only & False & Flag to only do simulation and rendering step \\
            --render-only & render\_only & False & Flag to only do rendering step\\
            --compress-only & compress\_only & False & Flag to only do compression step \\
            --reconstruct-only & reconstruct\_only & False & Flag to only do reconstruction step \\
            --compress-reconstruct-only & compress\_reconstruct\_only & False & Flag to only do compression and reconstruction step
        \end{tabular}
    }
\end{table}
