...
The precise shape and surface details of the \gls{sssb} are not provided as texture files and exact shape models. Blender shader nodes are used to create these during rendering. These shader nodes are combined in a network (cf. Figure \ref{fig:shader_nodes}). This approach of procedural terrain generation allows to use the same model on a large range of distances since the details are generated as needed.

\begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{doc/thesis/0_figures/star_rendering/Stars_2017-08-15T115856-171000.png}
    \caption{Overview of the shader nodes network in the Blender model used for creating a \gls{sssb} surface and terrain. Two main trees are visible in the network that are combined in the output, one for the shading and the other for displacement. Two main trees are interlinked.}
    \label{fig:shader_nodes}
\end{figure}

The shader implementation creates a material that combines surface shading and displacement.
Surface subtree

Displacement subtree
Add, Subtract, Mix Shader, Diffuse BSDF
