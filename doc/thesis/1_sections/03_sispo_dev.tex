...
The precise shape and surface details of the \gls{sssb} are not provided as texture files and exact shape models. Blender shader nodes are used to create these during rendering. These shader nodes are combined in a tree (cf. Figure \ref{fig:shader_nodes}). The tree consists of two main trees, one for shading and one for displacement, which are interlinked, i.e. displacement influences surface colour and vice-versa. This approach of procedural terrain generation allows to use the same model on a large range of distances since the details are generated as needed.

\begin{figure}[htb]
    \centering
    \includegraphics[width=\textwidth]{doc/thesis/0_figures/procedural_terrain/node_network.png}
    \caption{Overview of the shader nodes tree in the Blender model used for creating a \gls{sssb} surface and terrain. Two main trees are visible in the network that are combined in the output, one for shading and the other for displacement. The two main trees are interlinked.}
    \label{fig:shader_nodes}
\end{figure}

The shader implementation creates a material that combines surface shading and displacement. The used nodes are: 
Surface subtree

Displacement subtree
Add, Subtract, multiply, Mix Shader, Diffuse BSDF, RGB Curves, Noise texture, Voronoi texture, Mix, Principled BSDF, greater than, maximum, 
