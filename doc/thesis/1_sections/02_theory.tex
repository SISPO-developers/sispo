...
\end{align}
where $e_{i}$ is a vector for the $i^{th}$ edge of the \gls{fov}, $v_d$ is the direction vector of the camera, $v_j$ is the vector pointing right or up, $s_k$ is the camera sensor width or height and $f$ is the focal length.
...
The next step of photometric calibration is to calculate the reference number of photons per pixel of the \gls{ccd} $N_{ref}$ using
\begin{align}
    N_{ref} = F \times \frac{A \times A_{pixel}}{f^2 \times \pi}, \label{eq:comp_flux_pix}
\end{align}
where $F$ is the total photon flux, $A$ is the aperture area, $A_{pixel}$ is the area of a pixel and $f$ is the focal length of the instrument.

In the last step, the calibration factor is calculated. It is defined as
\begin{align}
    f_c = \frac{N_{ref} \times \alpha}{I_{ref}}, \label{eq:comp_cal_fac}
\end{align}
...
The hierarchical structure of an image transformed with \gls{dwt} is depicted in Figure 
\begin{figure}[htb]
    \centering
