...
A second problem was found in a scenario with \SI{50}{\kilo\meter} minimum distance and a \SI{1}{\kilo\meter} \gls{sssb}. A single image was found to be darker than all other images in the data set. A series of three images is shown in Figure \ref{fig:render_dark}. In total, three pixels are much brighter than any other pixel in the image. These are composed images for better visibility of the artefact. However, artefact exists in the raw OpenEXR SssbOnly image hence it is not introduced by the composition process. No second image with the same issue was found in any other data set.
...
\begin{figure}[htb]
    \centering
        ...
        \end{subfigure}
    \caption{Images with different level of compression using \gls{jpeg}2000 and one \gls{png}.}
    \label{fig:img_quality_comp}
\end{figure}

Difference images and histograms are used to show the effects of compression further. Difference images take the L2-norm of the \gls{rgb} values of each pixel and subtract them from the raw image. The result is a heat map showing differences through a colour scale. The heat maps for the different quality levels are shown in Figure \ref{fig:img_quality_heatmap}. The respective histograms are shown in Figure \ref{fig:img_quality_histogram}.
...
Figures \ref{fig:img_quality_heatmap} and \ref{fig:img_quality_histogram} that the \gls{jp2} quality levels 1000 does not differ from the  

\subsection{Reconstruction}
\gls{sispo} reconstructs using default parameters given in Table \ref{tab:comp_settings}. Refine options is set to NONE to make \gls{sfm} algorithms more likely to converge. It is assumed that in space missions, the intrinsic camera parameters can determined with sufficient accuracy before launch and therefore the parameters do not need to be optimised anymore.
